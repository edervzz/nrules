...
class SaveConditionsRuleModel:
    ...
    def __init__(self, j):
        ...
        raw_conditions = self.__dict__.get("insert_conditions", [])
        if isinstance(raw_conditions, list):
            for c in raw_conditions:
                ...
                self.insert_conditions.append(a_condition)

        raw_conditions = self.__dict__.get("update_conditions", [])
        if isinstance(raw_conditions, list):
            for c in raw_conditions:
                a_condition = Condition()
                a_condition.variable = c["variable"] if "variable" in c else ""
                ...
                self.update_conditions.append(a_condition)
