...
class UpdateCaseRuleValidator(Validator):
    ...
    def __validate__(self, request: UpdateCaseRuleRequest):
        ...
        for e in request.cases:
            if len(e.id) == 0:
                raise self.as_error(self.local.get(Codes.CASE_UPD_001))

            if e.position < 0:
                raise self.as_error(self.local.get(Codes.CASE_UPD_002))

            unique_pos.add(e.position)

        if len(unique_pos) != len(request.cases):
            raise self.as_error(self.local.get(Codes.CASE_UPD_003))
