...
class CreateKVItemsRuleValidator(Validator):
    """_summary_"""
    ...
    def __validate__(self, request: CreateKVItemsRuleRequest):
        ...
        if request.id == "" and request.name == "":
            self.as_error(self.localizer.get(Codes.KVI_CREA_001))

        if len(request.income_kvitems) == 0:
            self.as_error(self.localizer.get(Codes.KVI_CREA_002))
        ...
        if len(request.new_parameters) != len(request.income_kvitems):
